...
class FreeboxBackup {
  ...
  async restoreFromFile(filename) {
    try {
      ...
      if (backupData.data && backupData.metadata) {
        // Format avec métadonnées (ancien format Freebox wrapped)
        console.log('[FreeboxBackup] Format avec metadata détecté, extraction de data.guilds');
        configData = backupData.data;
        metadata = backupData.metadata;
      } else if (backupData.guilds) {
        // Format direct (format correct)
        console.log('[FreeboxBackup] Format direct détecté (guilds)');
        ...
        throw new Error('Format de sauvegarde non reconnu - ni data.guilds ni guilds trouvé');
      }
  ...
  async saveBackupFile(configData) {
    try {
      ...
      // Métadonnées pour logging (pas sauvegardées dans le fichier)
      const metadata = {
        timestamp: new Date().toISOString(),
      ...
      console.log(`[FreeboxBackup] Sauvegarde écrite: ${dest} (${metadata.guilds_count} serveurs, ${Math.round(metadata.data_size/1024)}KB)`);
