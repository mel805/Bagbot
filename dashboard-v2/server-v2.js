...
app.get('/api/me', async (req, res) => {
  ...
  // Vérifier si l'utilisateur est admin et l'ajouter automatiquement aux autorisés
  try {
    const userId = userData.userId;
    const FOUNDER_ID = '943487722738311219';
    
    // Récupérer les informations du membre depuis Discord
    const memberReq = https.request({
      hostname: 'discord.com',
      path: `/api/v10/guilds/${GUILD}/members/${userId}`,
      method: 'GET',
      headers: {
        'Authorization': `Bot ${DISCORD_TOKEN}`,
        'Content-Type': 'application/json'
      ...
      memberRes.on('end', () => {
        if (memberRes.statusCode === 200) {
          ...
          // Charger les rôles staff depuis la config
          const config = readConfig();
          const staffRoleIds = config.guilds?.[GUILD]?.staffRoleIds || [];
          
          // Vérifier si l'utilisateur est fondateur ou admin
          const isFounder = userId === FOUNDER_ID;
          const isAdmin = userRoles.some(roleId => staffRoleIds.includes(roleId));
          
          // Ajouter automatiquement les admins et le fondateur
          if (isFounder || isAdmin) {
            if (!allowedUsers.has(userId)) {
              allowedUsers.add(userId);
              console.log(`✅ [Auto-Auth] ${userData.username} (${isFounder ? 'Fondateur' : 'Admin'}) ajouté automatiquement`);
            }
          }
          ...
          res.json({
            ...
            discriminator: userData.discriminator,
            avatar: userData.avatar,
            isAuthorized: allowedUsers.has(userId)
          });
        } else {
          // Si erreur Discord, on envoie quand même la réponse de base
          res.json({
            userId: userData.userId,
            username: userData.username,
            discriminator: userData.discriminator,
            avatar: userData.avatar,
            isAuthorized: allowedUsers.has(userData.userId)
          });
        }
      });
    });
    ...
    memberReq.on('error', () => {
      // En cas d'erreur, on envoie la réponse de base
      res.json({
        userId: userData.userId,
        username: userData.username,
        discriminator: userData.discriminator,
...
// GET /api/admin/app-users - Récupérer les utilisateurs avec leurs détails complets
app.get('/api/admin/app-users', async (req, res) => {
  ...
  try {
    ...
    // Pour chaque utilisateur autorisé, récupérer ses infos Discord
    for (const userId of allowedUsers) {
      try {
        const memberInfo = await new Promise((resolve, reject) => {
          ...
          memberReq.on('error', () => resolve(null));
          memberReq.end();
        });
        ...
        if (memberInfo) {
          ...
          usersWithDetails.push({
            userId: userId,
            username: memberInfo.user?.global_name || memberInfo.user?.username || `User-${userId.slice(-4)}`,
            roles: userRoles,
            isFounder: isFounder,
            isAdmin: isAdmin,
            roleLabel: isFounder ? 'Fondateur' : (isAdmin ? 'Admin' : 'Membre')
          });
        }
      } catch (e) {
    ...
    console.error('[/api/admin/app-users] Error:', error);
...
// POST /api/admin/allowed-users/remove - Retirer un utilisateur (alternative au DELETE)
app.post('/api/admin/allowed-users/remove', express.json(), (req, res) => {
  const authHeader = req.headers['authorization'];
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    return res.status(401).json({ error: 'No token' });
  }
  ...
  // Ne pas permettre de retirer le fondateur
  if (userId === '943487722738311219') {
    return res.status(403).json({ error: 'Cannot remove founder' });
  }
